...
class ContrastiveHammingLoss(torch.nn.Module):
    def __init__(self, margin=2.0):
        super(ContrastiveHammingLoss, self).__init__()
        self.margin = margin

    def forward(self, output_1, output_2, label_lb):
        hamming_distance = torch.sum(torch.abs(output_1 - output_2))
        # print('------------------ hamming_distance', hamming_distance)
        contrastive_loss = torch.mean(label_lb * torch.pow(hamming_distance, 2) + (1 - label_lb) * torch.pow(torch.clamp(self.margin - hamming_distance, min=0.0), 2))
