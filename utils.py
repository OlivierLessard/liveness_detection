...
@preconditions(lambda v1, v2: len(list(v1)) == len(list(v2)))
def hammingDistance(v1, v2):
    # print('//////////////////// v1', v1)
    # print('//////////////////// v2', v2)
    # return sum(c1 != c2 for c1, c2 in zip(v1, v2))
    return torch.count_nonzero(v1 != v2, axis=1)


def fft_fun(image, no_peaks):
    _, _, M, N = image.shape
    image_low_pass_filter = gaussian_filter(image, sigma=1)
    image_high_pass_filter = image - image_low_pass_filter
    fft_components = torch.fft.fft(image_high_pass_filter)
